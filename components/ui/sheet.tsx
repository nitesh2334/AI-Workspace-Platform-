...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ref={ref}
      className={cn(
        "fixed z-50 gap-4 border-border bg-background p-6 shadow-2xl transition-transform duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]",
        side === "right" &&
          "inset-y-0 right-0 h-full w-[min(100%,20rem)] border-l sm:max-w-sm data-[state=closed]:translate-x-full data-[state=open]:translate-x-0",
        side === "left" &&
          "inset-y-0 left-0 h-full w-[min(100%,20rem)] border-r sm:max-w-sm data-[state=closed]:-translate-x-full data-[state=open]:translate-x-0",
        side === "top" &&
